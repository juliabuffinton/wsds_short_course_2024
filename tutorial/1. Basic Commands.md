...
1. Navigate to the folder where you want to set up git tracking in git bash. Note: you may need to create a new folder (`mdkir folder_name`) and navigate into it (`cd folder_name`) first! 
2. Type `git init`.
3. Check success:
...
5. Double check to make sure we edited what we needed to by comparing against the previous version we added. Use the `git diff` command to show the lines of text that were changed. You should see the line with Accountblity in red indicating it was removed, and the line with Accountability in green indicating it was added.
6. Add the README file to be staged for commit.
7. Commit the file with a descriptive message, such as `git commit -m "add initial headers to README file"`.